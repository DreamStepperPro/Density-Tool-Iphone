...
window.toggleComponent = function(id, name) {
    ...
    if (currentActiveDowntimes[id]) {
        ...
        if (!smartBtn) {
            ...
            const swapBtnRef = document.getElementById('btnSwapSeverity');
            if (swapBtnRef) swapBtnRef.parentNode.insertBefore(smartBtn, swapBtnRef.nextSibling);
        }
        if (smartBtn) {
...
// Calculates optimal cutter-to-lane routing when cutters are degraded.
// DSI_BASELINE maps each actuator to its default sub-lane and path.
// =====================================================================
const DSI_BASELINE = {
    1: { subLane: 1, path: 2 }, 2: { subLane: 1, path: 1 },
    3: { subLane: 2, path: 2 }, 4: { subLane: 2, path: 1 },
    5: { subLane: 3, path: 2 }, 6: { subLane: 3, path: 1 },
    7: { subLane: 4, path: 2 }, 8: { subLane: 4, path: 1 }
};
...
window.calculateSmartRoute = function(degradedCutterIds) {
    ...
    if (totalHealthy === 7) {
        ...
        subLaneAllocations = downCutter >= 5 ? { 1: 2, 2: 2, 3: 2, 4: 1 } : { 1: 1, 2: 2, 3: 2, 4: 2 };
    ...
    overlay.innerHTML = `
        <div style="background:var(--card-bg,#1e1e2e);border-radius:12px;width:100%;max-width:460px;padding:20px;box-shadow:0 8px 32px rgba(0,0,0,0.6);color:var(--text,#eee);font-family:inherit;max-height:90vh;overflow-y:auto;">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
                ...
                <button aria-label="Close Smart Route Overlay" onclick="document.getElementById('smartRouteOverlay').remove()"
...
window.enforceShiftBoundaries = function() {
    ...
    // Sandbox: wipe all machine memories and refresh current view
    if (window.isOfflineMode) {
        for (let i = 1; i <= cfg.machines; i++) {
            if (window.sandboxDowntimes[i]) window.sandboxDowntimes[i] = {};
        }
        currentActiveDowntimes = window.sandboxDowntimes[cfg.currentMachine] || {};
        window.syncMatrixToCloud();
        return;
    }
    ...
    // Use static get (already imported) — fetch all active downtimes across every machine at once
    get(ref(db, 'activeDowntimes')).then(snap => {
        const allDowntimes = snap.val() || {};
        for (let i = 1; i <= cfg.machines; i++) {
            const machKey     = `M${i}`;
            const machineFaults = allDowntimes[machKey] || {};
            const keys        = Object.keys(machineFaults);
            if (keys.length === 0) continue;
            keys.forEach(id => {
                const faultData    = machineFaults[id];
                const durationMins = Math.max(1, Math.round((now - faultData.startTime) / 60000));
                const permanentRecord = {
                    machine:    machKey,
                    component:  faultData.name,
                ...
                push(ref(db, 'downtimeLogs'), permanentRecord)
                    .then(() => set(ref(db, `activeDowntimes/${machKey}/${id}`), null));
            });
        }
    }).catch(e => console.warn("Auto-close cross-machine fetch failed:", e));
};

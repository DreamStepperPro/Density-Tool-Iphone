...
window.loadYieldHistory = function() {
    ...
    unsubYieldHistory = onValue(ref(db, 'yieldHistory'), (snap) => {
        ...
        const data = snap.val();
        if (!data) { list.innerHTML = `<div class="yield-hist-card" style="opacity:0.6; text-align:center; border-style:dashed;">${window.t('noYieldHistory') || 'No history saved yet.'}</div>`; return; }

...
window.renderHistoryCards = function() {
    ...
    if (!history || history.length === 0) {
        container.innerHTML = `<div style="text-align:center; opacity:0.5; padding:20px; font-size:0.85rem; background:var(--input-bg); border-radius:8px; border:1px dashed var(--border);">${window.t('noHistory') || 'No shift history yet. Save a check to begin.'}</div>`;
        return;
    }

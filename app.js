...
const ADMIN_UID    = 'VxWEghELhyOJ1D9Thgaj5vhDKxt1';
...
window.renderHistoryCards = function() {
    ...
    if (!history || history.length === 0) {
        const fallbackText = 'No shift history yet.';
        const emptyText = typeof window.t === 'function' ? (window.t('noHistory') || fallbackText) : fallbackText;
        container.innerHTML = `<div style="text-align:center; opacity:0.5; padding:20px; font-size:0.85rem;">${emptyText}</div>`;
        return;
    }
